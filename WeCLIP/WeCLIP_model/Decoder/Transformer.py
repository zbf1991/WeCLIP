    ...
    def __init__(self,
        ...
        for i in range(self.num_down_stages):
            level_match_layers = ModuleList([
                ...
                ConvModule(self.match_dims, self.match_dims, kernel_size=3, stride=1, padding=1, norm_cfg=dict(type="BN"))
        ...
        self.cls = nn.Sequential(
            ...
            nn.Conv2d(embed_dims * 4, embed_dims * 4, kernel_size=3, stride=1, padding=1),
            nn.SyncBatchNorm(embed_dims * 4),
            nn.Conv2d(embed_dims * 4, 21, kernel_size=1, stride=1, padding=0)
        )
...
class Transformer(nn.Module):
    ...
    def forward(self, features, supp_features, mask):
        ...
        outs, weights = self.mix_transformer(features, supp_features, mask)
        return outs, weights
