...
parser.add_argument("--model_path", default="work_dir_voc/checkpoints/2023-07-16-22-04/wetr_iter_30000.pth", type=str, help="model_path")
...
def validate(model, dataset, test_scales=None):
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
        ...
        # segs_cat, cls_pred, cam = model(inputs_cat, names, mode = 'val')
        # cls_pred = cls_pred[0]
        # cls_pred = torch.sigmoid(cls_pred)
        # cls_pred[cls_pred > 0.5] = 1
        # cls_pred[cls_pred <= 0.5] = 0
        
        # a = (cls_pred.cpu()-cls_labels.cpu()).sum()
        ...
        # print('cls_pred', cls_pred.flatten(0), 'cls_labels', cls_labels)
        ...
        for s in test_scales:
            if s != 1.0:
                ...
                # segs_cat, cls_pred_cat, cam_cat = model(inputs_cat, names, mode='val')
        ...
        # print('111', cam.shape, seg_preds.shape, labels.shape)
    ...
    return _gts, _preds, _msc_preds, cams
...
def main(cfg):
    ...
    gts, preds, msc_preds, cams = validate(model=wetr, dataset=val_dataset, test_scales=[1, 0.75])
