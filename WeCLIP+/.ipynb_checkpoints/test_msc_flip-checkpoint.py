...
from wetr.model_attn_aff_voc import Wetr_CLIP
...
parser.add_argument("--model_path", default="/root/autodl-tmp/FCLIP_DINO/work_dir_voc/checkpoints"
                                            "/2024-03-08-13-24/wetr_iter_30000.pth", type=str, help="model_path")
...
def validate(model, dataset, test_scales=None):
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
        ...
        # resized_inputs = torch.zeros([1,3,args.resize_long, args.resize_long]).cuda().float() 
        # resized_inputs[:,:,0:h, 0:w] = inputs[:,:,0:h, 0:w]
...
#         '''newadd'''
#         resized_segs = F.interpolate(segs, size=(args.resize_long, args.resize_long), mode='bilinear', align_corners=False)
#         resized_segs = resized_segs[:,:,0:h, 0:w]
...
def main(cfg):
    ...
    gts, preds, msc_preds, cams, preds_hist, msc_preds_hist, cams_hist = validate(model=wetr, dataset=val_dataset, test_scales=[1, 1.5]) #[1, 0.75]
