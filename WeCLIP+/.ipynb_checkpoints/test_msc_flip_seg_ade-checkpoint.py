...
from wetr.model_attn_aff_voc_seg import Wetr_CLIP
import imageio.v2 as imageio
parser = argparse.ArgumentParser()
parser.add_argument("--config",
                    default='configs/ade20k_attn_reg.yaml',
                    type=str,
                    help="config")
...
parser.add_argument("--model_path", default="/data1/zbf_data/Project2023_zbf/Myafa_CLIP/scripts/work_dir_ade/checkpoints"
                                            "/2023-09-30-17-15/wetr_iter_30000.pth", type=str, help="model_path")
# parser.add_argument("--model_path", default="work_dir_voc/wetr_iter_30000.pth", type=str, help="model_path")
...
def validate(model, dataset, test_scales=None):
    ...
    for idx, data in tqdm(enumerate(data_loader), total=len(data_loader), ncols=100, ascii=" >="):
        ...
        segs_cat = model(inputs_cat, names, mode = 'val')
        ...
        if num % 1000 == 0:
            _preds_hist, seg_score = evaluate.scores(_gts, _preds, _preds_hist,num_classes)
            _msc_preds_hist, msc_seg_score = evaluate.scores(_gts, _msc_preds, _msc_preds_hist,num_classes)
            _cams_hist, cam_score = evaluate.scores(_gts, cams, _cams_hist,num_classes)
            _preds, _gts, _msc_preds, cams = [], [], [], []
...
def crf_proc(config):
    ...
    images_path = os.path.join(config.dataset.root_dir, 'images/validation',)
    labels_path = os.path.join(config.dataset.root_dir, 'annotations/validation')
    ...
    hist, score = evaluate.scores(gts, preds, hist, num_classes)
...
def main(cfg):
    ...
    val_dataset = ade20k.ADE20KSegDataset(
    ...
    wetr = Wetr_CLIP(num_classes=cfg.dataset.num_classes,
    ...
    wetr.load_state_dict(state_dict=trained_state_dict, strict=False)
    wetr.eval()



    gts, preds, msc_preds, cams, preds_hist, msc_preds_hist, cams_hist = validate(model=wetr, dataset=val_dataset, test_scales=[1, 0.75])
    torch.cuda.empty_cache()

    preds_hist, seg_score = evaluate.scores(gts, preds, preds_hist,num_classes)
    msc_preds_hist, msc_seg_score = evaluate.scores(gts, msc_preds, msc_preds_hist,num_classes)
    cams_hist, cam_score = evaluate.scores(gts, cams, cams_hist,num_classes)
