...
class DecoderTransformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, output_dim: int):
        ...
        # self.cls_pred = nn.Conv2d(width, output_dim-1, kernel_size=1)
        self.pooling = nn.AdaptiveMaxPool1d(1)


    def forward(self, x: torch.Tensor, cls_x: torch.Tensor):
        b, c, h, w = x.shape
        cls_b, cls_c, cls_h, cls_w = cls_x.shape
        x = x.reshape(b, c, h*w) # NDL
        x = x.permute(2, 0, 1)  # NDL -> LND
        cls_x = cls_x.reshape(b, c, -1)
        cls_x = cls_x.permute(2, 0, 1) # (20, bs, channel)
        
        x_all = torch.cat([cls_x, x], dim=0)
        
        # x_all, attn_weights_list = self.transformer(x_all) # L,N,D
        x_all, attn_weights_list = self.transformer(x_all) # L,N,D
        
        cls_attn_weight_list = []
        x_attn_weight_list = []
        for attn_weight in attn_weights_list:
            cls_attn_weight = attn_weight[:, :cls_h, cls_h:]
            x_attn_weight = attn_weight[:, cls_h:, cls_h:]
            cls_attn_weight_list.append(cls_attn_weight)
            x_attn_weight_list.append(x_attn_weight)
        
        
        cls_x = x_all[:cls_h,: ,:]
        x = x_all[cls_h:,:,: ]
        
        # print('222', x_all.shape, cls_x.shape, x.shape)
        
        x = x.permute(1, 2, 0)
        x = x.reshape(b, c, h, w)
        logit = self.linear_pred(x)
        
        cls_x = cls_x.permute(1, 0, 2) # (4, 20, 256)
        cls_logit = self.pooling(cls_x)
        
        # cls_x = cls_x.reshape(cls_b, cls_c, cls_h, cls_w)
        # cls_logit = self.cls_pred(cls_x)

        return logit, cls_logit, x_attn_weight_list, cls_attn_weight_list